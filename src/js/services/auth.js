...
export async function updateAccount(changes){if(isDemoMode){const user=await currentUser();const updated={...user,user_metadata:{...user.user_metadata,...changes}};localStorage.setItem(demoKey,JSON.stringify(updated));return updated}const{data,error}=await supabase.auth.updateUser({data:changes});if(error)throw error;if(changes.full_name)await supabase.from('profiles').update({full_name:changes.full_name}).eq('id',data.user.id);return data.user}
